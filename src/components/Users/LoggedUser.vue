<template>
	<div class="links col-12 col-md-8">
		<div class="al-menu clearfix">
			...
			<!-- Search -->
			<a style="margin-right: 2rem; margin-top: 2rem; display: block;">
				<!--<i class="fas fa-plus"></i>KULLANICI <span>ARA!</span>-->
				<input class="form-control" type="text" placeholder="Kullanıcı Ara" v-model="usr"
				       v-on:keydown.enter.prevent="searchUser"/>
			</a>


			<!-- new coupon button -->
			<router-link to="/coupon" class="btn-new-coupon"><i class="fas fa-plus"></i>YENİ KUPON <span>YAP!</span>
			</router-link>
				...
				<div class="dropdown-menu dropdown-menu-right notification-menu">
					...
					<ul>
						<li v-for="not in notifications" v-if="notifications.length > 0">
							<router-link :to="`${not.bildirim_url}`" class="clearfix"
							             @mouseover="read(not.bildirim_id)">
			...
			<div class="user-menu-al float-left" @mouseover="userMenuOver" @mouseleave="userMenuOut">
				...
				<div data-toggle="show" data-target="#login-after-menu" :onmouseover="userMenuOver"
				     :onmouseout="userMenuOut">
					<div class="clearfix">
					...
					<ul class="user-menu-list" id="login-after-menu" v-show="userMenuActive">
						...
						<li>
							<router-link to="/coupon">Kupon Yap</router-link>
						</li>
...
<script>
	export default {
		...
		methods: {
			searchUser() {
				this.$router.push("/search/"+this.usr);
				/*
				this.searchDisabled = true;
				this.$store.dispatch("users/searchUser", {username: this.usr}).then(res => {
